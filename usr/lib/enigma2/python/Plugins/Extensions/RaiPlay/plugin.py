		...
		try:
			...
			for station in tv_stations:
				...
				if title:
					...
					self.icons.append(self.api.getFullUrl(icon_url if icon_url else station.get('icon', "")))
	...
	def _gotPageLoad(self):
		...
		try:
			...
			for item in items:
				...
				self.icons.append(icon_url)  # CRITICAL: Populate icons list for poster widget
		...
		if sub_type == "Raiplay Tipologia Item":
			...
			if program_data:
				...
				if is_movie and program_data['info'].get("first_item_path"):
					self.playDirect(name, program_data['info']["first_item_path"])
				else:
					self.session.open(

        ...
        try:
            ...
            if "items" in response and isinstance(response["items"], list):
                for i, item in enumerate(response["items"]):
                    ...
                    item_data = {
                        "name": item.get(
                            "name", ""), "url": url_fixed, "icon": icon_url if icon_url else self.getFullUrl(
                            item.get(
                                "images", {}).get(
                                "landscape", "")), "sub-type": item.get(
                            "type", item.get(
                                "sub_type", ""))}
                    ...
                    if key in response and response[key]:
                        for item in response[key]:
                            ...
                            item_data = {
                                'name': name,
                                'url': url_fixed,
                                'icon': icon_url if icon_url else self.getFullUrl(
                                    item.get(
                                        "images",
                                        {}).get(
                                        "landscape",
            ...
            for item in items:
                ...
                video_info = {
                    'title': item.get(
                        ...
                        "pathID", ""), 'icon': icon_url if icon_url else self.getFullUrl(
                        item.get(
                            "images", {}).get(
...
class RaiPlayMain(SafeScreen):
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['OkCancelActions',], {
                                    'ok': self.okRun, 'cancel': self.closerm}, -1)
        ...
        try:
            ...
            for item in items:
                ...
                if item.get("images", {}).get("portrait", ""):
                    icon_url = self.api.getThumbnailUrl2(
